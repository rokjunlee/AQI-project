...
### Load Data
I will show the first 5 rows of the raw data below.
...
### Reshaping Data, aqi
Comments explain why I select certain columns and why I drop the other columns. Data type conversions are also performed below. Basically, I am creating a useful/amenable dataset for me to explore.
```{r, warning=FALSE}
aqi <- aqi %>%
...
aqi <- aqi %>% 
  select(-c(temperature, dewpoint, humidity)) #dropping repetitive columns
# same information is contained in tem, dew, humid!
# That is why the columns with the longer names are dropped above
...
### Labeling pollutant columns
Unit conversion will need to be conducted later to convert the pollutants' concentrations into AQI scores, using the formula given on page 11 of the following pdf file published by the EPA: https://www3.epa.gov/airnow/aqi-technical-assistance-document-may2016.pdf. I am going to put units that THIS data set uses right below the column names for now.
```{r}
...
label(aqi$so2) <- "pphm"
label(aqi$no2) <- "pphm"
# labels are visible if you use the View(aqi) command
```

### Quick Facts about aqi!
Basic and simple exploration of the dataset, aqi.
```{r}
...
# I do not think this number is too concerning, considering that there are 1590 observations and 16 variables
...
### About Pollutants - by hour
Under "Quick facts about aqi" above, I showed that this dataset is collected over 4 different times. I organized information about pollutants, by time.
```{r}
#pollutants by time
aqi %>% 
  group_by(hour) %>% 
  summarise(
    ...
    avg_no2 = mean(no2, na.rm = TRUE),  #using RAW concentrations
    avg_so2 = mean(so2, na.rm = TRUE),
    avg_co = mean(co, na.rm = TRUE)
    )
#but since the pollutants are measured in different units, it would be much sensible to compare their mean concentrations after scaling them like the following:
aqi %>% 
  group_by(hour) %>% 
...
#As you can see, after scaling, pm25 has the greatest concentration at 4 different times.
...
However, AQI scores (later) will tell us that it is actually another pollutant that is responsible for raising AQI score. The higher the AQI, the worse.

Also, the units given in the data is not the same as units used for calculating the AQI score by the EPA. Therefore, the above plots are misleading.


### About Pollutants - by pollutants!
Statistics for each of the 6 pollutants can be found below.
```{r, warning=FALSE}
...
```
<br>
Again from the plots above, pm25 'seems' like the most problematic pollutant.
As aforementioned, the units given in the data is not the same as units used for calculating the AQI score. Therefore, the above plots are misleading.


### Quick check on Multicorrelinearity
I am showing correlation matrix, and the maximum correlation.
```{r, echo=FALSE}
...
```
<br>
The maximum correlation is 0.7294517. The rest are not very concerning.
Overall, multicollinearity does **not** seem to be a major problem.

### Let's calculate AQI scores.
First, some unit conversions are conducted to use EPA's formula. Then several numerical vectors that contain appropriate breakpoints are created for each pollutants. Depending on the two breakpoints that each concentration falls in, each concentration is assigned fitting categories: "very good", "good", "moderate", "bad", "very bad", "dangerous", "toxic". Of course, these categories indicate the quality of the air. Some lengthy functions are created to convert the concentrations to AQI scores. 
...
aqi <- aqi %>% 
  mutate(
    ...
    no2 = 10*no2)
label(aqi$o3) <- "ppm"
label(aqi$so2) <- "ppb"     #labelling CORRECT units for AQI conversion
label(aqi$no2) <- "ppb"
...
# Assigning correct categories according to concentrations
...
aqi[c("pm25", "pm25_cat", "pm10", "pm10_cat")] # concentration and its category for pm25 and pm10
...
### 'The' AQI score
Currently, we have AQI scores for each of the 6 pollutants. Then for each row, which AQI score are we supposed to use? The answer is the the maximum AQI score. That is, for each row we identify the maximum AQI score, and that score is 'the' final AQI score for that area at that particular time.
```{r}
...
aqi_score   #notice the two NEW columns that have been added on the right
...
### Lastly, some EDA using AQI scores
Before we calculated the AQI score, it looked as is pm25 is the most harmful pollutant. Let us see if that is true with AQI scores.
```{r}
aqi %>% 
  group_by(max_pollut) %>% 
...
#From above, o3's AQI score was used for final AQI score most of the time, followed by pm25

#Below we observe that the point estimate for the o3's AQI score is the highest. 
aqi %>% 
...
#Notice that there is not a single observation that fall into the "very good" category
...
aqi %>% 
  ...
  coord_flip() +
  theme_bw()
#Notice the dominance of o3's AQI score as the category gets worse!
```
<br>
Overall, o3 seems to be the most problematic pollutant. This contradicts our initial few plots where we concluded that pm25 seemed to be most problematic one. BUT for the "bad" and "moderate" categories, pm25 does seem to be the major pollutant.


### Preparing for Statistical Analysis
If I want to do clustering or PCA, I will use the entire data set: aqi.
If I want to do classification, I will need a training set and testing set. I decided to use 85% of my data for training, as you can see below. Also, from now on, I will  always use lucky **three** for seed.
```{r}
set.seed(3)
#Training set: aqi_train
aqi_train <- aqi %>% 
  sample_frac(0.85)    #85% <- I will use 85% of the data as the training set

#Testing set: aqi_test
aqi_test <- aqi %>% 
  setdiff(aqi_train)

#Let's check thier dimensions
dim(aqi)       #1590   31
