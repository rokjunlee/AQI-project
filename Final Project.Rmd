...
The following table summarizes the detrimental health effects that each pollutants can cause on different groups of people. Notice that Ozone, $O_3$, affects the largest number of number of groups.
...
I conducted a simple neural network where I only use **6 predictors** and have **two layers**. The six predictors are the AQI scores of each pollutants. The first layer contains 12 hidden units, and the second layer contains 7 units, because there are 7 categories. You can see that the validation accuracy reaches about 60%, considering how there are 7 classes and only 6 predictors are used.
